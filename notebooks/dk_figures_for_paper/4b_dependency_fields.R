...
countries_flows <- df %>% 
  ...
  group_by(origin,for_division) %>% 
  mutate(n_origin = sum(n_origin_destination)) %>% 
  group_by(destination,for_division) %>% 
...
countries_as_destinations <- function(df,countries_flows,selection_countries){
  ...
  selection_df <- countries_flows %>% 
    ...
    group_by(for_division,destination,origin_region) %>% #for each country of destinations
  ...
  selection_df %>% 
    ggplot(aes(origin_region,for_division, fill=fields, alpha=p_origin))+
    geom_tile() +
...
countries_as_origins <- function(df,countries_flows,selection_countries){
  ...
  selection_df <- countries_flows %>% 
    ...
    group_by(for_division,origin,destination_region) %>% #for each country of destinations
  ...
  selection_df %>% 
    ggplot(aes(destination_region,for_division, fill=fields, alpha=p_destination))+
    geom_tile() +
    # scale_y_continuous(labels = percent,limits = c(0,0.45))+
    coord_flip()+
    tidytext::scale_x_reordered() +
    facet_grid2(origin~., scales = 'free',strip = strip)+
    labs(x='as destination', alpha='%', fill='',y='')+
    scale_alpha_binned(labels=percent)+
    theme_minimal()+
    theme(legend.position = 'bottom')
}
